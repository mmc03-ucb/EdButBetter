...
function Insights({ subsection, onBack }) {
  ...
  const { cacheInsights, getCachedInsights } = useCache();
  ...
  const fetchAndSummarizeThreads = async () => {
    try {
      ...
      const cachedInsights = getCachedInsights(subsection);
      
      if (cachedInsights) {
        setAISummary(cachedInsights);
        setLoading(false);
        return;
      }
  ...
  const generateAISummary = async (threads) => {
    try {
      ...
      cacheInsights(subsection, data.summary);
